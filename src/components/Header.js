...
const Header = ({ 
  ...
  const menuItems = [
    { path: '/daily', label: 'Hôm nay', icon: 'today', fillWhenActive: true },
    { path: '/goals', label: 'Mục tiêu', icon: 'target', fillWhenActive: true },
      ...
      <div className="lg:hidden flex border-t border-gray-100 bg-white overflow-x-auto">
        {menuItems.map((item) => (
          <Link 
            key={item.path}
            to={item.path} 
            className={`flex-1 min-w-[100px] flex items-center justify-center gap-2 py-3 font-medium text-sm whitespace-nowrap px-4 ${
              isActive(item.path)
                ? 'text-primary border-b-2 border-primary'
                : 'text-gray-500 hover:text-gray-900'
            }`}
          >
            <span className={`material-symbols-outlined text-lg ${isActive(item.path) && item.fillWhenActive ? 'fill-1' : ''}`}>
              {item.icon}
            </span>
            {item.label}
          </Link>
